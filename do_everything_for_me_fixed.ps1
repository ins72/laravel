...
Write-Host "`nStep 1: Stopping all running servers..." -ForegroundColor Yellow
try {
    # Stop PHP processes
...
Write-Host "`nStep 2: Configuring XAMPP MySQL for port 5003..." -ForegroundColor Yellow
$xamppPath = "C:\xampp"
...
Write-Host "`nStep 3: Updating Laravel configuration..." -ForegroundColor Yellow
try {
    if (Test-Path ".env") {
...
Write-Host "`nStep 4: Running Laravel migrations..." -ForegroundColor Yellow
try {
...
Write-Host "`nStep 5: Clearing Laravel caches..." -ForegroundColor Yellow
try {
    ...
    Write-Host "Cache clearing may have failed" -ForegroundColor Yellow
}

# Step 6: Start Laravel server in background
Write-Host "`nStep 6: Starting Laravel server..." -ForegroundColor Yellow
try {
...
Write-Host "`nStep 7: Starting Vite server..." -ForegroundColor Yellow
try {
...
Write-Host "`nStep 8: Waiting for servers to start..." -ForegroundColor Yellow
Start-Sleep -Seconds 10

# Step 9: Test connections
Write-Host "`nStep 9: Testing connections..." -ForegroundColor Yellow
try {
    # Test MySQL
    $mysqlPath = "$xamppPath\mysql\bin\mysql.exe"
    ...
    Write-Host "MySQL connection test failed" -ForegroundColor Yellow
}

# Step 10: Show final status
Write-Host "`nEverything is Done!" -ForegroundColor Green
Write-Host "=====================" -ForegroundColor Green
...
Write-Host "`nTo stop everything later, run: Get-Process -Name 'php','node','mysqld' | Stop-Process -Force" -ForegroundColor Yellow 